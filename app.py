import hashlib
import re
import time
from datetime import datetime, timezone
from urllib.parse import urljoin, urlparse

import pandas as pd
import requests
import streamlit as st
from bs4 import BeautifulSoup
from supabase import create_client


# ============================================================
# CONFIGURAZIONE
# ============================================================

SOURCES = {
    "Forlì": "https://forli.bakecaincontrii.com/donna-cerca-uomo/",
    "Rimini": "https://rimini.bakecaincontrii.com/donna-cerca-uomo/",
    "Ravenna": "https://ravenna.bakecaincontrii.com/donna-cerca-uomo/",
}

ADS_TABLE = "annunci"
PHONE_TABLE = "telefoni_registry"

REQUEST_TIMEOUT = 20

MAX_PAGES_PER_CITY = 10

LISTING_PAGE_DELAY = 0.20
DETAIL_PAGE_DELAY = 0.25

# Supabase restituisce normalmente max 1000 righe per pagina
SUPABASE_PAGE_SIZE = 1000

# ============================================================
# SERPER
# ============================================================

SERPER_MAX_PER_CLICK = 5
SERPER_DELAY = 2.0
SERPER_MAX_RETRIES = 4
SERPER_BACKOFF_BASE = 3


HEADERS = {
    "User-Agent": (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/128.0 Safari/537.36"
    ),
    "Accept-Language": "it-IT,it;q=0.9,en;q=0.7",
}


# ============================================================
# UTILITA
# ============================================================

def utc_now_iso():
    return datetime.now(
        timezone.utc
    ).isoformat()


def now_label():
    return datetime.now().strftime(
        "%H:%M:%S"
    )


def format_datetime(value):
    if not value:
        return "-"

    try:
        dt = datetime.fromisoformat(
            str(value).replace(
                "Z",
                "+00:00"
            )
        )

        return dt.strftime(
            "%d/%m/%Y %H:%M"
        )

    except Exception:
        return str(value)


def clean_text(value):
    if not value:
        return ""

    return re.sub(
        r"\s+",
        " ",
        value
    ).strip()


def clean_domain(value):
    if not value:
        return ""

    value = str(
        value
    ).strip().lower()

    if "://" not in value:
        value = (
            "https://"
            + value
        )

    hostname = (
        urlparse(
            value
        ).hostname
        or ""
    )

    return hostname.lower().strip(".")


# ============================================================
# TELEFONO
# ============================================================

def normalize_phone(value):
    if not value:
        return ""

    value = str(
        value
    ).replace(
        "tel:",
        ""
    )

    digits = re.sub(
        r"\D",
        "",
        value
    )

    if digits.startswith(
        "0039"
    ):
        digits = digits[4:]

    if (
        digits.startswith("39")
        and len(digits) >= 12
    ):
        digits = digits[2:]

    if not (
        8 <= len(digits) <= 11
    ):
        return ""

    return digits


def make_phone_hash(phone):
    normalized = normalize_phone(
        phone
    )

    if not normalized:
        return ""

    return hashlib.sha256(
        normalized.encode(
            "utf-8"
        )
    ).hexdigest()


def extract_phone_from_detail(html):
    soup = BeautifulSoup(
        html,
        "html.parser"
    )

    candidates = []

    # --------------------------------------------------------
    # href="tel:"
    # --------------------------------------------------------

    for a in soup.select(
        'a[href^="tel:"]'
    ):
        candidates.append(
            a.get(
                "href",
                ""
            )
        )

    # --------------------------------------------------------
    # Attributi HTML
    # --------------------------------------------------------

    phone_attrs = (
        "data-phone",
        "data-telephone",
        "data-tel",
        "data-number",
        "data-phone-number",
        "phone",
        "telephone",
    )

    for tag in soup.find_all(
        True
    ):
        for attr in phone_attrs:

            if tag.has_attr(
                attr
            ):
                candidates.append(
                    str(
                        tag.get(
                            attr
                        )
                    )
                )

    # --------------------------------------------------------
    # Titolo
    # --------------------------------------------------------

    if soup.title:
        candidates.append(
            soup.title.get_text(
                " ",
                strip=True
            )
        )

    # --------------------------------------------------------
    # Meta
    # --------------------------------------------------------

    for meta in soup.find_all(
        "meta"
    ):
        content = meta.get(
            "content"
        )

        if content:
            candidates.append(
                content
            )

    # --------------------------------------------------------
    # Script / JSON
    # --------------------------------------------------------

    for script in soup.find_all(
        "script"
    ):
        text = (
            script.string
            or script.get_text(
                " ",
                strip=True
            )
        )

        if text:
            candidates.append(
                text
            )

    # --------------------------------------------------------
    # Testo visibile
    # --------------------------------------------------------

    candidates.append(
        soup.get_text(
            " ",
            strip=True
        )
    )

    mobile_pattern = re.compile(
        r"(?<!\d)"
        r"(?:\+?39[\s.\-]*)?"
        r"(3(?:[\s.\-]*\d){8,9})"
        r"(?!\d)"
    )

    for candidate in candidates:

        if not candidate:
            continue

        match = mobile_pattern.search(
            candidate
        )

        if match:

            phone = normalize_phone(
                match.group(0)
            )

            if phone:
                return phone

    return ""


# ============================================================
# ANNUNCI
# ============================================================

def looks_like_ad_url(href):
    if not href:
        return False

    absolute = urljoin(
        "https://www.bakecaincontrii.com/",
        href
    )

    parsed = urlparse(
        absolute
    )

    path = (
        parsed.path.lower()
    )

    if "/annuncio/" in path:
        return True

    blocked_parts = (
        "/donna-cerca-uomo/",
        "/uomo-cerca-uomo/",
        "/trans/",
        "/incontri/",
        "/pubblica",
        "/privacy",
        "/cookie",
        "/termin",
        "/contatt",
        "/assistenza",
    )

    return (
        parsed.netloc.endswith(
            "bakecaincontrii.com"
        )
        and not any(
            part in path
            for part in blocked_parts
        )
        and len(
            path.strip("/")
        ) > 20
    )


def extract_listing_links(
    html,
    page_url
):
    soup = BeautifulSoup(
        html,
        "html.parser"
    )

    results = []
    seen_urls = set()

    for a in soup.find_all(
        "a",
        href=True
    ):

        href = a.get(
            "href"
        )

        if not looks_like_ad_url(
            href
        ):
            continue

        url = urljoin(
            page_url,
            href
        )

        if url in seen_urls:
            continue

        title = clean_text(
            a.get_text(
                " ",
                strip=True
            )
        )

        if len(title) < 5:

            img = a.find(
                "img",
                alt=True
            )

            if img:
                title = clean_text(
                    img.get(
                        "alt"
                    )
                )

        if (
            len(title) < 5
            and a.parent
        ):

            heading = a.parent.find(
                [
                    "h1",
                    "h2",
                    "h3",
                    "h4"
                ]
            )

            if heading:
                title = clean_text(
                    heading.get_text(
                        " ",
                        strip=True
                    )
                )

        if len(title) < 5:
            continue

        seen_urls.add(
            url
        )

        results.append(
            {
                "titolo":
                    title,

                "url":
                    url,
            }
        )

    return results


def get_page(
    session,
    url
):
    response = session.get(
        url,
        timeout=REQUEST_TIMEOUT
    )

    response.raise_for_status()

    return response.text


# ============================================================
# SERPER CON RETRY
# ============================================================

def serper_domain_matches(
    phone,
    target_domain,
    max_results=20,
    log_callback=None,
):
    if not phone:
        return 0, []

    if (
        "SERPER_API_KEY"
        not in st.secrets
    ):
        raise RuntimeError(
            "SERPER_API_KEY non configurata."
        )

    target_domain = clean_domain(
        target_domain
    )

    if not target_domain:
        raise RuntimeError(
            "TARGET_DOMAIN non valido."
        )

    url = (
        "https://google.serper.dev/search"
    )

    headers = {
        "X-API-KEY":
            st.secrets[
                "SERPER_API_KEY"
            ],

        "Content-Type":
            "application/json",
    }

    payload = {
        "q":
            str(phone),

        "num":
            min(
                int(max_results),
                20
            ),

        "gl":
            "it",

        "hl":
            "it",
    }

    last_error = None

    for attempt in range(
        1,
        SERPER_MAX_RETRIES + 1
    ):

        try:

            response = requests.post(
                url,
                headers=headers,
                json=payload,
                timeout=REQUEST_TIMEOUT,
            )

            # ------------------------------------------------
            # 429
            # ------------------------------------------------

            if response.status_code == 429:

                retry_after = (
                    response.headers.get(
                        "Retry-After"
                    )
                )

                if retry_after:

                    try:
                        wait_seconds = float(
                            retry_after
                        )

                    except ValueError:
                        wait_seconds = (
                            SERPER_BACKOFF_BASE
                            * (
                                2
                                ** (
                                    attempt - 1
                                )
                            )
                        )

                else:

                    wait_seconds = (
                        SERPER_BACKOFF_BASE
                        * (
                            2
                            ** (
                                attempt - 1
                            )
                        )
                    )

                last_error = (
                    "429 Too Many Requests"
                )

                if log_callback:

                    log_callback(
                        f"{now_label()} - "
                        f"429, tentativo "
                        f"{attempt}/"
                        f"{SERPER_MAX_RETRIES}. "
                        f"Attendo "
                        f"{wait_seconds:.0f}s"
                    )

                time.sleep(
                    wait_seconds
                )

                continue

            # ------------------------------------------------
            # Errori temporanei server
            # ------------------------------------------------

            if response.status_code in (
                500,
                502,
                503,
                504,
            ):

                wait_seconds = (
                    SERPER_BACKOFF_BASE
                    * (
                        2
                        ** (
                            attempt - 1
                        )
                    )
                )

                last_error = (
                    f"HTTP "
                    f"{response.status_code}"
                )

                if log_callback:

                    log_callback(
                        f"{now_label()} - "
                        f"Errore server "
                        f"{response.status_code}, "
                        f"tentativo "
                        f"{attempt}/"
                        f"{SERPER_MAX_RETRIES}. "
                        f"Attendo "
                        f"{wait_seconds:.0f}s"
                    )

                time.sleep(
                    wait_seconds
                )

                continue

            response.raise_for_status()

            data = (
                response.json()
            )

            organic = (
                data.get(
                    "organic",
                    []
                )[:max_results]
            )

            links = []
            seen = set()

            for item in organic:

                link = item.get(
                    "link",
                    ""
                )

                if not link:
                    continue

                hostname = (
                    urlparse(
                        link
                    ).hostname
                    or ""
                ).lower()

                if (
                    hostname == target_domain
                    or hostname.endswith(
                        "."
                        + target_domain
                    )
                ):

                    if link not in seen:

                        seen.add(
                            link
                        )

                        links.append(
                            link
                        )

            return (
                len(links),
                links
            )

        except requests.RequestException as exc:

            last_error = exc

            if (
                attempt
                < SERPER_MAX_RETRIES
            ):

                wait_seconds = (
                    SERPER_BACKOFF_BASE
                    * (
                        2
                        ** (
                            attempt - 1
                        )
                    )
                )

                if log_callback:

                    log_callback(
                        f"{now_label()} - "
                        f"Errore rete, "
                        f"tentativo "
                        f"{attempt}/"
                        f"{SERPER_MAX_RETRIES}. "
                        f"Attendo "
                        f"{wait_seconds:.0f}s"
                    )

                time.sleep(
                    wait_seconds
                )

                continue

    raise RuntimeError(
        "Serper non disponibile dopo "
        f"{SERPER_MAX_RETRIES} tentativi. "
        f"Ultimo errore: "
        f"{last_error}"
    )


# ============================================================
# SUPABASE
# ============================================================

@st.cache_resource
def get_supabase():

    return create_client(
        st.secrets[
            "SUPABASE_URL"
        ],

        st.secrets[
            "SUPABASE_KEY"
        ],
    )


# ============================================================
# LETTURA SUPABASE PAGINATA
# ============================================================

def fetch_all_rows(
    table_name,
    columns="*",
    order_by=None,
    desc=False,
    page_size=SUPABASE_PAGE_SIZE,
):
    """
    Legge TUTTE le righe di una tabella Supabase
    usando range() a blocchi da 1000.

    Risolve il problema del limite standard
    delle prime 1000 righe.
    """

    db = get_supabase()

    all_rows = []

    start = 0

    while True:

        end = (
            start
            + page_size
            - 1
        )

        query = (
            db
            .table(
                table_name
            )
            .select(
                columns
            )
        )

        if order_by:

            query = query.order(
                order_by,
                desc=desc
            )

        response = (
            query
            .range(
                start,
                end
            )
            .execute()
        )

        rows = (
            response.data
            or []
        )

        all_rows.extend(
            rows
        )

        if len(rows) < page_size:
            break

        start += page_size

    return all_rows


# ============================================================
# ANNUNCI DB
# ============================================================

def load_all_ads():

    return fetch_all_rows(
        table_name=ADS_TABLE,
        columns="*",
        order_by="first_seen",
        desc=True,
    )


def load_known_ads():

    rows = fetch_all_rows(
        table_name=ADS_TABLE,
        columns=(
            "url,"
            "telefono,"
            "phone_hash"
        ),
    )

    result = {}

    for row in rows:

        url = row.get(
            "url"
        )

        if url:

            result[
                url
            ] = {

                "telefono":
                    row.get(
                        "telefono"
                    )
                    or "",

                "phone_hash":
                    row.get(
                        "phone_hash"
                    )
                    or "",
            }

    return result


def insert_new_ad(row):

    (
        get_supabase()
        .table(
            ADS_TABLE
        )
        .insert(
            row
        )
        .execute()
    )


def update_last_seen(
    url,
    seen_at
):

    (
        get_supabase()
        .table(
            ADS_TABLE
        )
        .update(
            {
                "last_seen":
                    seen_at,

                "active":
                    True,
            }
        )
        .eq(
            "url",
            url
        )
        .execute()
    )


def update_ad_phone(
    url,
    phone,
    phone_hash
):
    """
    Ripara un annuncio già salvato
    che in precedenza non aveva telefono/hash.
    """

    (
        get_supabase()
        .table(
            ADS_TABLE
        )
        .update(
            {
                "telefono":
                    phone,

                "phone_hash":
                    phone_hash,
            }
        )
        .eq(
            "url",
            url
        )
        .execute()
    )


def mark_missing_as_inactive(
    current_urls
):

    db = get_supabase()

    # Ora legge TUTTO il database,
    # non soltanto le prime 1000 righe.
    for row in load_all_ads():

        url = row.get(
            "url"
        )

        if (
            url
            and url not in current_urls
            and row.get(
                "active",
                True
            )
        ):

            (
                db
                .table(
                    ADS_TABLE
                )
                .update(
                    {
                        "active":
                            False
                    }
                )
                .eq(
                    "url",
                    url
                )
                .execute()
            )


# ============================================================
# REGISTRO TELEFONI
# ============================================================

def load_phone_registry():

    rows = fetch_all_rows(
        table_name=PHONE_TABLE,
        columns="*",
    )

    return {

        row[
            "phone_hash"
        ]:
            row

        for row in rows

        if row.get(
            "phone_hash"
        )
    }


def insert_phone_registry(
    phone_hash,
    seen_at
):

    (
        get_supabase()
        .table(
            PHONE_TABLE
        )
        .insert(
            {
                "phone_hash":
                    phone_hash,

                "first_seen":
                    seen_at,

                "last_seen":
                    seen_at,

                "processed":
                    False,

                "target_domain":
                    None,

                "match_count":
                    0,

                "match_urls":
                    [],

                "processed_at":
                    None,

                "note":
                    "",

                "hidden":
                    False,

                "hidden_at":
                    None,
            }
        )
        .execute()
    )


def update_phone_last_seen(
    phone_hash,
    seen_at
):

    if not phone_hash:
        return

    (
        get_supabase()
        .table(
            PHONE_TABLE
        )
        .update(
            {
                "last_seen":
                    seen_at
            }
        )
        .eq(
            "phone_hash",
            phone_hash
        )
        .execute()
    )


def save_phone_note(
    phone_hash,
    note
):

    (
        get_supabase()
        .table(
            PHONE_TABLE
        )
        .update(
            {
                "note":
                    note
                    or ""
            }
        )
        .eq(
            "phone_hash",
            phone_hash
        )
        .execute()
    )


def set_phone_hidden(
    phone_hash,
    hidden
):

    (
        get_supabase()
        .table(
            PHONE_TABLE
        )
        .update(
            {
                "hidden":
                    bool(hidden),

                "hidden_at":
                    (
                        utc_now_iso()
                        if hidden
                        else None
                    ),
            }
        )
        .eq(
            "phone_hash",
            phone_hash
        )
        .execute()
    )


# ============================================================
# SALVA RISULTATO SERPER
# ============================================================

def save_external_check(
    phone_hash,
    target_domain,
    match_count,
    match_urls
):

    (
        get_supabase()
        .table(
            PHONE_TABLE
        )
        .update(
            {
                "processed":
                    True,

                "target_domain":
                    clean_domain(
                        target_domain
                    ),

                "match_count":
                    int(
                        match_count
                    ),

                "match_urls":
                    match_urls,

                "processed_at":
                    utc_now_iso(),
            }
        )
        .eq(
            "phone_hash",
            phone_hash
        )
        .execute()
    )


# ============================================================
# PAGINAZIONE SITO
# ============================================================

def build_listing_page_url(
    base_url,
    page_number
):

    if page_number <= 1:
        return base_url

    return (
        f"{base_url}"
        f"?p={page_number}"
    )


# ============================================================
# SCRAPING
# ============================================================

def scrape_and_sync():

    session = requests.Session()

    session.headers.update(
        HEADERS
    )

    # Queste funzioni ora leggono TUTTE le righe
    known_ads = (
        load_known_ads()
    )

    phone_registry = (
        load_phone_registry()
    )

    current_urls = set()

    new_ads = 0
    known_ads_count = 0
    new_phones = 0
    duplicate_phones = 0

    # Annunci vecchi che erano senza telefono/hash
    recovered_phones = 0
    recovery_failed = 0

    pages_loaded = 0
    pages_failed = 0

    errors = []

    now = utc_now_iso()

    for (
        city,
        source_url
    ) in SOURCES.items():

        city_listings = []
        seen_city_urls = set()

        # ====================================================
        # PRIME 10 PAGINE
        # ====================================================

        for page_number in range(
            1,
            MAX_PAGES_PER_CITY + 1
        ):

            page_url = (
                build_listing_page_url(
                    source_url,
                    page_number
                )
            )

            try:

                listing_html = get_page(
                    session,
                    page_url
                )

                page_listings = (
                    extract_listing_links(
                        listing_html,
                        page_url
                    )
                )

                pages_loaded += 1

            except requests.RequestException as exc:

                pages_failed += 1

                errors.append(
                    f"{city} "
                    f"pagina {page_number}: "
                    f"{exc}"
                )

                continue

            # Deduplica all'interno della città
            for item in page_listings:

                if (
                    item["url"]
                    in seen_city_urls
                ):
                    continue

                seen_city_urls.add(
                    item["url"]
                )

                city_listings.append(
                    item
                )

            time.sleep(
                LISTING_PAGE_DELAY
            )

        # ====================================================
        # PROCESSA ANNUNCI
        # ====================================================

        for item in city_listings:

            url = item[
                "url"
            ]

            current_urls.add(
                url
            )

            # =================================================
            # URL GIÀ PRESENTE NEL DATABASE
            # =================================================

            if url in known_ads:

                known_ads_count += 1

                try:

                    update_last_seen(
                        url,
                        now
                    )

                except Exception as exc:

                    errors.append(
                        f"Aggiornamento "
                        f"{url}: {exc}"
                    )

                existing_phone = (
                    normalize_phone(
                        known_ads[
                            url
                        ].get(
                            "telefono",
                            ""
                        )
                    )
                )

                existing_hash = (
                    known_ads[
                        url
                    ].get(
                        "phone_hash",
                        ""
                    )
                    or ""
                )

                # ---------------------------------------------
                # Record completo: non riapriamo il dettaglio
                # ---------------------------------------------

                if (
                    existing_phone
                    and existing_hash
                ):

                    try:

                        update_phone_last_seen(
                            existing_hash,
                            now
                        )

                    except Exception as exc:

                        errors.append(
                            "Aggiornamento telefono "
                            f"{existing_hash[:8]}: "
                            f"{exc}"
                        )

                    continue

                # ---------------------------------------------
                # BUG FIX:
                # URL conosciuto ma manca telefono / hash.
                #
                # Riapriamo il dettaglio e tentiamo
                # di riparare il record.
                # ---------------------------------------------

                try:

                    detail_html = get_page(
                        session,
                        url
                    )

                    recovered_phone = (
                        extract_phone_from_detail(
                            detail_html
                        )
                    )

                    recovered_hash = (
                        make_phone_hash(
                            recovered_phone
                        )
                    )

                    if (
                        recovered_phone
                        and recovered_hash
                    ):

                        # Aggiorna annuncio esistente
                        update_ad_phone(
                            url=url,
                            phone=recovered_phone,
                            phone_hash=recovered_hash,
                        )

                        # Registra telefono se non esiste
                        if (
                            recovered_hash
                            not in phone_registry
                        ):

                            insert_phone_registry(
                                recovered_hash,
                                now
                            )

                            phone_registry[
                                recovered_hash
                            ] = {

                                "phone_hash":
                                    recovered_hash,

                                "first_seen":
                                    now,

                                "last_seen":
                                    now,

                                "processed":
                                    False,

                                "target_domain":
                                    None,

                                "match_count":
                                    0,

                                "match_urls":
                                    [],

                                "processed_at":
                                    None,

                                "note":
                                    "",

                                "hidden":
                                    False,

                                "hidden_at":
                                    None,
                            }

                            new_phones += 1

                        else:

                            update_phone_last_seen(
                                recovered_hash,
                                now
                            )

                        # Aggiorna anche la memoria locale
                        known_ads[
                            url
                        ] = {

                            "telefono":
                                recovered_phone,

                            "phone_hash":
                                recovered_hash,
                        }

                        recovered_phones += 1

                    else:

                        recovery_failed += 1

                except Exception as exc:

                    recovery_failed += 1

                    errors.append(
                        "Recupero telefono "
                        f"{url}: {exc}"
                    )

                time.sleep(
                    DETAIL_PAGE_DELAY
                )

                continue

            # =================================================
            # NUOVO URL
            # =================================================

            phone = ""
            phone_hash = ""

            try:

                detail_html = get_page(
                    session,
                    url
                )

                phone = (
                    extract_phone_from_detail(
                        detail_html
                    )
                )

                phone_hash = (
                    make_phone_hash(
                        phone
                    )
                )

            except requests.RequestException as exc:

                errors.append(
                    f"Dettaglio "
                    f"{url}: {exc}"
                )

            # =================================================
            # REGISTRO TELEFONO
            # =================================================

            if phone_hash:

                if (
                    phone_hash
                    in phone_registry
                ):

                    duplicate_phones += 1

                    try:

                        update_phone_last_seen(
                            phone_hash,
                            now
                        )

                    except Exception as exc:

                        errors.append(
                            "Telefono esistente: "
                            f"{exc}"
                        )

                else:

                    try:

                        insert_phone_registry(
                            phone_hash,
                            now
                        )

                        phone_registry[
                            phone_hash
                        ] = {

                            "phone_hash":
                                phone_hash,

                            "first_seen":
                                now,

                            "last_seen":
                                now,

                            "processed":
                                False,

                            "target_domain":
                                None,

                            "match_count":
                                0,

                            "match_urls":
                                [],

                            "processed_at":
                                None,

                            "note":
                                "",

                            "hidden":
                                False,

                            "hidden_at":
                                None,
                        }

                        new_phones += 1

                    except Exception as exc:

                        errors.append(
                            "Nuovo telefono: "
                            f"{exc}"
                        )

            # =================================================
            # SALVA NUOVO ANNUNCIO
            # =================================================

            row = {

                "url":
                    url,

                "titolo":
                    item[
                        "titolo"
                    ],

                "citta":
                    city,

                "telefono":
                    phone
                    or "",

                "phone_hash":
                    phone_hash
                    or None,

                "first_seen":
                    now,

                "last_seen":
                    now,

                "active":
                    True,

                "note":
                    "",

                "external_check_done":
                    False,

                "external_match_count":
                    None,
            }

            try:

                insert_new_ad(
                    row
                )

                known_ads[
                    url
                ] = {

                    "telefono":
                        phone,

                    "phone_hash":
                        phone_hash,
                }

                new_ads += 1

            except Exception as exc:

                errors.append(
                    "Salvataggio annuncio "
                    f"{url}: {exc}"
                )

            time.sleep(
                DETAIL_PAGE_DELAY
            )

    # ========================================================
    # INATTIVI
    # ========================================================

    expected_pages = (
        len(SOURCES)
        * MAX_PAGES_PER_CITY
    )

    if (
        current_urls
        and pages_failed == 0
        and pages_loaded
        == expected_pages
    ):

        try:

            mark_missing_as_inactive(
                current_urls
            )

        except Exception as exc:

            errors.append(
                "Aggiornamento inattivi: "
                f"{exc}"
            )

    elif pages_failed > 0:

        errors.append(
            "Almeno una pagina elenco non è "
            "stata caricata. Gli annunci mancanti "
            "non sono stati marcati come inattivi."
        )

    return {

        "new_ads":
            new_ads,

        "known_ads":
            known_ads_count,

        "new_phones":
            new_phones,

        "duplicate_phones":
            duplicate_phones,

        "recovered_phones":
            recovered_phones,

        "recovery_failed":
            recovery_failed,

        "pages_loaded":
            pages_loaded,

        "pages_failed":
            pages_failed,

        "errors":
            errors,
    }


# ============================================================
# TELEFONO DA PROCESSARE?
# ============================================================

def needs_match_check(
    registry_row,
    target_domain
):

    if not registry_row:
        return True

    if registry_row.get(
        "hidden",
        False
    ):
        return False

    processed = bool(
        registry_row.get(
            "processed",
            False
        )
    )

    stored_domain = clean_domain(
        registry_row.get(
            "target_domain",
            ""
        )
    )

    current_domain = clean_domain(
        target_domain
    )

    if not processed:
        return True

    if (
        stored_domain
        != current_domain
    ):
        return True

    return False


# ============================================================
# CALCOLA MATCH - MAX 5 PER CLICK
# ============================================================

def calculate_pending_matches(
    target_domain,
    log_callback
):

    # Ora entrambe leggono TUTTO il DB
    known_ads = (
        load_known_ads()
    )

    registry = (
        load_phone_registry()
    )

    phones_by_hash = {}

    # ========================================================
    # COSTRUISCE phone_hash -> telefono
    # ========================================================

    for ad in known_ads.values():

        phone = normalize_phone(
            ad.get(
                "telefono",
                ""
            )
        )

        phone_hash = (
            ad.get(
                "phone_hash",
                ""
            )
            or ""
        )

        if (
            phone
            and phone_hash
            and phone_hash
            not in phones_by_hash
        ):

            phones_by_hash[
                phone_hash
            ] = phone

    # ========================================================
    # TROVA TUTTI I PENDING
    # ========================================================

    pending = []

    for (
        phone_hash,
        phone
    ) in phones_by_hash.items():

        row = registry.get(
            phone_hash,
            {}
        )

        if needs_match_check(
            row,
            target_domain
        ):

            pending.append(
                (
                    phone_hash,
                    phone
                )
            )

    total_pending = len(
        pending
    )

    # Solo 5 per click
    batch = pending[
        :SERPER_MAX_PER_CLICK
    ]

    success_count = 0
    error_count = 0

    # ========================================================
    # LOG INIZIALE
    # ========================================================

    log_callback(
        f"{now_label()} - "
        f"Telefoni da processare trovati: "
        f"{total_pending}"
    )

    # ========================================================
    # PROCESSA BATCH
    # ========================================================

    for (
        index,
        (
            phone_hash,
            phone
        )
    ) in enumerate(
        batch,
        start=1
    ):

        log_callback(
            f"{now_label()} - "
            f"Query "
            f"{index}/"
            f"{len(batch)} "
            f"per ID "
            f"{phone_hash[:8]}"
        )

        try:

            (
                match_count,
                match_urls
            ) = serper_domain_matches(
                phone=phone,
                target_domain=target_domain,
                max_results=20,
                log_callback=log_callback,
            )

            save_external_check(
                phone_hash,
                target_domain,
                match_count,
                match_urls
            )

            success_count += 1

            log_callback(
                f"{now_label()} - "
                f"Completato "
                f"{phone_hash[:8]} "
                f"- match: "
                f"{match_count}"
            )

        except Exception as exc:

            error_count += 1

            # IMPORTANTE:
            # non viene chiamato save_external_check()
            # quindi processed resta FALSE
            log_callback(
                f"{now_label()} - "
                f"ERRORE "
                f"{phone_hash[:8]}: "
                f"{exc}"
            )

        time.sleep(
            SERPER_DELAY
        )

    remaining_estimate = (
        total_pending
        - success_count
    )

    return {

        "pending_before":
            total_pending,

        "attempted":
            len(batch),

        "success":
            success_count,

        "errors":
            error_count,

        "remaining":
            max(
                0,
                remaining_estimate
            ),
    }


# ============================================================
# VISTA RAGGRUPPATA
# ============================================================

def build_grouped_view(
    ads_rows,
    registry_rows,
    show_hidden=False
):

    registry = {

        row[
            "phone_hash"
        ]:
            row

        for row in registry_rows

        if row.get(
            "phone_hash"
        )
    }

    df = pd.DataFrame(
        ads_rows
    )

    if df.empty:
        return []

    if "active" not in df.columns:

        df[
            "active"
        ] = True

    # Solo annunci attivi
    df = df[
        df[
            "active"
        ] == True
    ].copy()

    # Solo annunci con hash
    df = df[
        df[
            "phone_hash"
        ]
        .fillna("")
        .astype(str)
        .str.len()
        > 0
    ]

    groups = []

    for (
        phone_hash,
        group
    ) in df.groupby(
        "phone_hash"
    ):

        reg = registry.get(
            phone_hash,
            {}
        )

        hidden = bool(
            reg.get(
                "hidden",
                False
            )
        )

        if (
            hidden
            and not show_hidden
        ):
            continue

        cities = sorted(
            {
                str(city)

                for city in (
                    group[
                        "citta"
                    ]
                    .dropna()
                )

                if str(
                    city
                ).strip()
            }
        )

        ads = []

        for _, row in (
            group.iterrows()
        ):

            ads.append(
                {
                    "titolo":
                        row.get(
                            "titolo"
                        )
                        or "Annuncio",

                    "url":
                        row.get(
                            "url"
                        )
                        or "",
                }
            )

        match_urls = (
            reg.get(
                "match_urls"
            )
            or []
        )

        if not isinstance(
            match_urls,
            list
        ):
            match_urls = []

        groups.append(
            {
                "telefono_id":
                    phone_hash[:8],

                "phone_hash":
                    phone_hash,

                "first_seen":
                    reg.get(
                        "first_seen"
                    ),

                "citta":
                    ", ".join(
                        cities
                    ),

                "annunci":
                    ads,

                "numero_annunci":
                    len(
                        ads
                    ),

                "match_count":
                    int(
                        reg.get(
                            "match_count"
                        )
                        or 0
                    ),

                "match_urls":
                    match_urls,

                "processed":
                    bool(
                        reg.get(
                            "processed",
                            False
                        )
                    ),

                "note":
                    reg.get(
                        "note"
                    )
                    or "",

                "hidden":
                    hidden,
            }
        )

    groups.sort(
        key=lambda x: (
            x[
                "match_count"
            ],

            x[
                "numero_annunci"
            ],

            str(
                x[
                    "first_seen"
                ]
                or ""
            ),
        ),
        reverse=True,
    )

    return groups


# ============================================================
# STREAMLIT
# ============================================================

st.set_page_config(
    page_title=(
        "Archivio annunci Romagna"
    ),
    page_icon="🔎",
    layout="wide",
)


# ============================================================
# CSS
# ============================================================

st.markdown(
    """
    <style>

    .block-container {
        padding-top: 1.5rem;
        padding-bottom: 2rem;
    }

    div[data-testid="stVerticalBlockBorderWrapper"] {
        border-color: rgba(
            128,
            128,
            128,
            0.50
        );
    }

    div[data-testid="stButton"] button {
        min-height: 2.4rem;
    }

    </style>
    """,
    unsafe_allow_html=True,
)


st.title(
    "🔎 Archivio annunci "
    "Forlì, Rimini e Ravenna"
)


# ============================================================
# SECRETS
# ============================================================

required_secrets = (
    "SUPABASE_URL",
    "SUPABASE_KEY",
    "SERPER_API_KEY",
    "TARGET_DOMAIN",
)


missing = [

    key

    for key in (
        required_secrets
    )

    if key not in (
        st.secrets
    )
]


if missing:

    st.error(
        "Mancano i Secrets: "
        + ", ".join(
            missing
        )
    )

    st.stop()


TARGET_DOMAIN = clean_domain(
    st.secrets[
        "TARGET_DOMAIN"
    ]
)


if not TARGET_DOMAIN:

    st.error(
        "TARGET_DOMAIN non valido."
    )

    st.stop()


st.caption(
    f"Scraping: prime "
    f"{MAX_PAGES_PER_CITY} pagine "
    f"per ciascuna città. "
    f"Supabase: lettura paginata completa. "
    f"Match: max "
    f"{SERPER_MAX_PER_CLICK} "
    f"query per click."
)


# ============================================================
# PULSANTI PRINCIPALI
# ============================================================

button_col1, button_col2 = (
    st.columns(2)
)


# ============================================================
# SCRAPING
# ============================================================

with button_col1:

    scrape_button = st.button(
        "🔄 Cerca nuovi annunci",
        type="primary",
        use_container_width=True
    )


if scrape_button:

    try:

        with st.spinner(
            "Scraping in corso. "
            "Controllo anche gli annunci "
            "già salvati che non hanno "
            "telefono/hash..."
        ):

            result = (
                scrape_and_sync()
            )

        st.success(
            f"Pagine lette: "
            f"{result['pages_loaded']}/"
            f"{len(SOURCES) * MAX_PAGES_PER_CITY} · "
            f"Nuovi annunci: "
            f"{result['new_ads']} · "
            f"Già conosciuti: "
            f"{result['known_ads']} · "
            f"Nuovi telefoni: "
            f"{result['new_phones']} · "
            f"Telefoni recuperati: "
            f"{result['recovered_phones']} · "
            f"Recovery senza risultato: "
            f"{result['recovery_failed']}"
        )

        if result[
            "errors"
        ]:

            with st.expander(
                f"Avvisi / errori "
                f"({len(result['errors'])})"
            ):

                for error in (
                    result[
                        "errors"
                    ]
                ):

                    st.write(
                        error
                    )

    except Exception as exc:

        st.error(
            f"Errore scraping: {exc}"
        )


# ============================================================
# CALCOLA MATCH
# ============================================================

with button_col2:

    calculate_match = st.button(
        f"🔎 Calcola match "
        f"(max {SERPER_MAX_PER_CLICK})",
        use_container_width=True
    )


if calculate_match:

    log_box = st.empty()

    logs = []

    def add_log(message):

        logs.append(
            message
        )

        log_box.code(
            "\n".join(
                logs
            )
        )

    try:

        result = (
            calculate_pending_matches(
                TARGET_DOMAIN,
                add_log
            )
        )

        st.success(
            f"Da processare all'inizio: "
            f"{result['pending_before']} · "
            f"Tentati: "
            f"{result['attempted']} · "
            f"Completati: "
            f"{result['success']} · "
            f"Errori: "
            f"{result['errors']} · "
            f"Ancora da processare: "
            f"{result['remaining']}"
        )

    except Exception as exc:

        st.error(
            f"Errore match: {exc}"
        )


# ============================================================
# CARICAMENTO DATI COMPLETO E PAGINATO
# ============================================================

try:

    ads_rows = (
        load_all_ads()
    )

    phone_registry_dict = (
        load_phone_registry()
    )

    registry_rows = list(
        phone_registry_dict.values()
    )

except Exception as exc:

    st.error(
        f"Errore Supabase: {exc}"
    )

    st.stop()


# ============================================================
# MOSTRA NASCOSTI
# ============================================================

show_hidden = (
    st.checkbox(
        "👁 Mostra anche risultati nascosti",
        value=False
    )
)


groups = (
    build_grouped_view(
        ads_rows,
        registry_rows,
        show_hidden
    )
)


if not groups:

    st.info(
        "Nessun risultato."
    )

    st.stop()


# ============================================================
# METRICHE
# ============================================================

m1, m2, m3, m4, m5 = (
    st.columns(5)
)


m1.metric(
    "Telefoni",
    len(
        groups
    )
)


m2.metric(
    "Con match",
    sum(
        1

        for group in groups

        if group[
            "match_count"
        ] > 0
    )
)


m3.metric(
    "Match totali",
    sum(
        group[
            "match_count"
        ]

        for group in groups
    )
)


# ============================================================
# PENDING CORRETTO
# ============================================================

pending_count = sum(

    1

    for row in registry_rows

    if needs_match_check(
        row,
        TARGET_DOMAIN
    )
)


m4.metric(
    "Da verificare",
    pending_count
)


missing_hash_count = sum(

    1

    for row in ads_rows

    if not (
        row.get(
            "phone_hash"
        )
        or ""
    )
)


m5.metric(
    "Annunci senza hash",
    missing_hash_count
)


# ============================================================
# TABELLA
# ============================================================

COLUMN_WIDTHS = [
    1.0,
    1.3,
    1.1,
    3.0,
    0.7,
    2.3,
    2.0,
    1.1,
]


headers = [
    "Telefono ID",
    "First seen",
    "Città",
    "Annunci",
    "Match",
    "Link esterni",
    "Note",
    "Azione",
]


# ============================================================
# HEADER
# ============================================================

with st.container(
    border=True
):

    cols = st.columns(
        COLUMN_WIDTHS,
        gap="small"
    )

    for (
        col,
        label
    ) in zip(
        cols,
        headers
    ):

        with col:

            with st.container(
                border=True
            ):

                st.markdown(
                    f"**{label}**"
                )


# ============================================================
# RIGHE
# ============================================================

for group in groups:

    phone_hash = (
        group[
            "phone_hash"
        ]
    )

    with st.container(
        border=True
    ):

        cols = st.columns(
            COLUMN_WIDTHS,
            gap="small"
        )

        # ====================================================
        # ID
        # ====================================================

        with cols[0]:

            with st.container(
                border=True
            ):

                st.code(
                    group[
                        "telefono_id"
                    ],
                    language=None
                )

                if group[
                    "hidden"
                ]:

                    st.caption(
                        "🙈 Nascosto"
                    )

        # ====================================================
        # FIRST SEEN
        # ====================================================

        with cols[1]:

            with st.container(
                border=True
            ):

                st.write(
                    format_datetime(
                        group[
                            "first_seen"
                        ]
                    )
                )

        # ====================================================
        # CITTA
        # ====================================================

        with cols[2]:

            with st.container(
                border=True
            ):

                st.write(
                    group[
                        "citta"
                    ]
                    or "-"
                )

        # ====================================================
        # ANNUNCI
        # ====================================================

        with cols[3]:

            with st.container(
                border=True
            ):

                for (
                    index,
                    ad
                ) in enumerate(
                    group[
                        "annunci"
                    ],
                    start=1
                ):

                    if ad[
                        "url"
                    ]:

                        st.markdown(
                            f"{index}. "
                            f"[{ad['titolo']}]"
                            f"({ad['url']})"
                        )

        # ====================================================
        # MATCH
        # ====================================================

        with cols[4]:

            with st.container(
                border=True
            ):

                if group[
                    "processed"
                ]:

                    st.markdown(
                        f"### "
                        f"{group['match_count']}"
                    )

                else:

                    st.caption(
                        "Da fare"
                    )

        # ====================================================
        # LINK ESTERNI
        # ====================================================

        with cols[5]:

            with st.container(
                border=True
            ):

                if group[
                    "match_urls"
                ]:

                    for (
                        index,
                        link
                    ) in enumerate(
                        group[
                            "match_urls"
                        ],
                        start=1
                    ):

                        st.markdown(
                            f"[Risultato "
                            f"{index}]({link})"
                        )

                elif group[
                    "processed"
                ]:

                    st.caption(
                        "Nessun risultato"
                    )

                else:

                    st.caption(
                        "Non verificato"
                    )

        # ====================================================
        # NOTE
        # ====================================================

        with cols[6]:

            with st.container(
                border=True
            ):

                note_value = (
                    st.text_area(
                        "Nota",
                        value=group[
                            "note"
                        ],
                        key=(
                            "note_"
                            + phone_hash
                        ),
                        label_visibility=(
                            "collapsed"
                        ),
                        height=80
                    )
                )

                if st.button(
                    "💾 Salva nota",
                    key=(
                        "save_note_"
                        + phone_hash
                    ),
                    use_container_width=True
                ):

                    try:

                        save_phone_note(
                            phone_hash,
                            note_value
                        )

                        st.rerun()

                    except Exception as exc:

                        st.error(
                            f"Errore nota: "
                            f"{exc}"
                        )

        # ====================================================
        # NASCONDI / RIPRISTINA
        # ====================================================

        with cols[7]:

            with st.container(
                border=True
            ):

                if group[
                    "hidden"
                ]:

                    if st.button(
                        "👁 Ripristina",
                        key=(
                            "restore_"
                            + phone_hash
                        ),
                        use_container_width=True
                    ):

                        try:

                            set_phone_hidden(
                                phone_hash,
                                False
                            )

                            st.rerun()

                        except Exception as exc:

                            st.error(
                                f"Errore: "
                                f"{exc}"
                            )

                else:

                    if st.button(
                        "🙈 Nascondi",
                        key=(
                            "hide_"
                            + phone_hash
                        ),
                        use_container_width=True
                    ):

                        try:

                            set_phone_hidden(
                                phone_hash,
                                True
                            )

                            st.rerun()

                        except Exception as exc:

                            st.error(
                                f"Errore: "
                                f"{exc}"
                            )


# ============================================================
# CSV
# ============================================================

csv_rows = []


for group in groups:

    csv_rows.append(
        {
            "telefono_id":
                group[
                    "telefono_id"
                ],

            "first_seen":
                group[
                    "first_seen"
                ],

            "citta":
                group[
                    "citta"
                ],

            "numero_annunci":
                group[
                    "numero_annunci"
                ],

            "match_count":
                group[
                    "match_count"
                ],

            "note":
                group[
                    "note"
                ],

            "hidden":
                group[
                    "hidden"
                ],

            "annunci":
                " | ".join(
                    (
                        f"{ad['titolo']} "
                        f"({ad['url']})"
                    )

                    for ad in (
                        group[
                            "annunci"
                        ]
                    )
                ),

            "match_urls":
                " | ".join(
                    group[
                        "match_urls"
                    ]
                ),
        }
    )


csv_data = (
    pd.DataFrame(
        csv_rows
    )
    .to_csv(
        index=False
    )
    .encode(
        "utf-8-sig"
    )
)


st.download_button(
    "📥 Scarica CSV",
    data=csv_data,
    file_name=(
        "annunci_raggruppati.csv"
    ),
    mime="text/csv"
)
